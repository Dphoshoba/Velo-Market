...
const ProductGrid: React.FC<ProductGridProps> = ({ products, onProductClick, onAddToCart }) => {
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
    ...
    // Attempt one high-res reliable fallback
    ...
    if (target.src !== secondaryFallback) {
      ...
      // If even the fallback fails, show a stylized artisan placeholder
      ...
      if (target.parentElement) {
        ...
        // Clear children to avoid text repetition
        ...
        >
          <div className="aspect-[4/3] overflow-hidden relative bg-slate-100">
            ...
            <div className="absolute top-4 left-4 flex gap-2">
               <div className="bg-white/95 backdrop-blur px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-900 shadow-sm border border-slate-100">
                {product.category}
              </div>
              {product.rating >= 4.9 && (
                <div className="bg-indigo-600 text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-600/20">
                  Elite Craft
                </div>
              )}
            </div>

            {product.image?.includes('base64') && (
              <div className="absolute bottom-4 left-4">
                 <div className="bg-slate-900/40 backdrop-blur-md px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest text-white/90 border border-white/10 flex items-center gap-1.5">
                    <div className="w-1.5 h-1.5 bg-indigo-400 rounded-full animate-pulse"></div>
                    AI Conceptual Render
                 </div>
              </div>
            )}
          </div>
          ...
          <div className="p-8 flex flex-col flex-grow">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="font-black text-slate-900 group-hover:text-indigo-600 transition-colors text-lg leading-tight mb-1">{product.name}</h3>
